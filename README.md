...
- Periodo: cada asignatura pertenece a un semestre (`period_season` O=Otoño, P=Primavera y `period_year`). La combinación `code+section+period` es única y está disponible en los endpoints como `period_code` (ej. `O-2025`). El periodo por defecto se define con la variable de entorno `SUBJECT_DEFAULT_PERIOD` (por ejemplo `P-2025`). Las acciones `by-code` aceptan `?period=O-2025` (o `?period_year=&period_season=`) y, si no se envía, utilizan el periodo configurado en esa variable.
